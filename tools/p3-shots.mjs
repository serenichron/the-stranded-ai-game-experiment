// Phase 3 (agent C) fixed shot list for the world half, same cameras as phase 2. One browser, one session, every shot.
// Usage: node tools/p3-shots.mjs <port> <round> [shot,shot,...]
//   Shoots a FROZEN build (vite preview), never the user's dev server on 5190.
//   Saves test-output/p3-C-<round>-<shot>.png and prints fps per shot.
...
const ME = 'agent C (tools/p3-shots.mjs)';
...
try {
  ...
  for (const [name, s] of Object.entries(SHOTS)) {
    ...
    const out = `test-output/p3-C-${round}-${name}.png`;
